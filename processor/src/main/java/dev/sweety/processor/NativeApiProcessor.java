...
public final class NativeApiProcessor extends AbstractProcessor {
    ...
    private static final String ENGINE_BASE = """
            ...
            %direct%
                ...
                @Override public %sessionName% open() { return sessions.acquire(); }
                @Override public Backend backend() { return backend; }
                @Override public Binding binding() { return Binding.%bindingEnum%; }
    ...
    private void writeEngineBase(String pkg, Engine engine, String prefix, String bindingEnum,
        ...
        String directMethods = join(direct, m ->
                "    @Override public " + m.segmentReturn() + " " + m.name + "(" + m.segmentParams() + ") {\n"
                + "        " + (m.ret == Kind.VOID ? "" : "return ") + "b." + m.name + "(" + m.segmentArgs() + ");\n"
                + "    }");
        ...
        for (Marshal.Strategy role : List.of(Marshal.Strategy.SESSION_UPDATE,
            ...
            sm.append("        @Override public ").append(m.segmentReturn()).append(" ").append(m.name)
              .append("(").append(m.sessionParams()).append(") {\n")
              .append("            ").append(m.ret == Kind.VOID ? "" : "return ")
              .append("b.").append(m.name).append("(").append(m.sessionArgs()).append(");\n")
              .append("        }");
        }
    ...
    private void writeJniBindings(String pkg, String[] names, String[] enums, List<Method> jni) throws IOException {
        ...
        for (int i = 0; i < names.length; i++) {
            if (i > 0) cases.append("\n");
            cases.append("            case ").append(enums[i]).append(" -> ").append(names[i]).append("Natives.INSTANCE;");
        }
    ...
    private static final class Method {
        ...
        final Marshal.Strategy strategy; // nullable: binding-only

        Method(String name, Kind ret, List<Kind> params, Jni jni, Cabi cabi, Marshal.Strategy strategy) {
            this.name = name;
            this.ret = ret;
            this.params = params;
            this.jni = jni;
