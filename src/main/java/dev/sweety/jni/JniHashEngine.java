...
public final class JniHashEngine implements HashEngine {
    ...
    public JniHashEngine(Backend backend) {
        ...
        this.sessions = ObjectPool.threadLocal(
                () -> new JniSession(n.sCreate()),
                s -> n.sReset(s.handle),    // reset native state on return
                s -> n.sFree(s.handle),     // free on pool overflow
                16);
    }

    @Override public long hash(byte[] data) { return n.hashArray(data); }

    @Override public long hash(MemorySegment data, long len) {
        return n.hashAddr(data.address(), len);
    }
    ...
    @Override public long[] hashBatch(MemorySegment[] data, long[] lens) {
        ...
        return n.hashBatch(addrs, lens);
    }

    @Override public void transform(MemorySegment data, long len, byte add) {
        n.transformAddr(data.address(), len, add);
    }

    @Override public HashSession open() {
        return sessions.acquire();
    }

    @Override public Backend backend() { return backend; }
    @Override public dev.sweety.Binding binding() { return dev.sweety.Binding.JNI; }

    /** Hashes a heap array without copying, via GetPrimitiveArrayCritical. */
    public long hashCritical(byte[] data) { return n.hashArrayCritical(data); }

    final class JniSession implements HashSession {
        final long handle;

        JniSession(long handle) { this.handle = handle; }

        @Override public void update(MemorySegment data, long len) {
            n.sUpdate(handle, data.address(), len);
        }
        @Override public long digest() { return n.sDigest(handle); }
        @Override public void reset() { n.sReset(handle); }
